...
mME <- mutate(mME, group=substr(treatment, 0, 1))
mME$group <- gsub('m','Mutant', mME$group)
mME$group <- gsub('w','Wild Type', mME$group)
mod_treat_heatmap <- mME %>% ggplot(., aes(x=factor(treatment, level = level_order), y=name, fill=value)) +
  ...
  theme_classic(base_size = 10, base_family='Arial Black')+
  facet_grid(~group, scale='free')+
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  labs(title = "Module-trait Relationships", y = "Modules", fill="corr", x='Sample')


ggsave(plot=mod_treat_heatmap, file='figs/treatment_heatmap.final.png', device='png', dpi=1300, width=6.00, height=6.00, units='in')
...
#import gene/module list
module_df <- read_delim('data/gene_modules.nofilter.txt',delim='\t')
...
submod_df$genotype <- gsub('m', 'Mutant', submod_df$genotype)
submod_df$genotype <- gsub('w', 'Wild Type', submod_df$genotype)

#plot expression levels amongst treatments for specific modules
mod_expr_plt <- submod_df %>% ggplot(., aes(x=as.factor(time), y=value, group=gene_id)) +
  geom_line(aes(color = genotype),
            alpha = 0.2) +
  theme_bw(base_size = 8, base_family='Arial Black')+ 
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  facet_grid(rows = vars(module), cols = vars(genotype)) +
  labs(x = "Treatment",
       y = "Normalized Expression",
       color='Genotype')

ggsave(plot=mod_expr_plt, file='figs/module_norm_expression.nofilter.png', device='png', dpi=1300, width=6.00, height=6.00, units='in')
